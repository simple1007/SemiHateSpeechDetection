...
class ModelBuild(tf.keras.Model):
    def __init__(self,maxlen):
        ...
        self.bilstm = Bidirectional(LSTM(64,return_sequences=True,name="lstm"),name="bidirectional")#(emb)
...
if True:
    ...
    callbacks = [EarlyStopping(monitor='val_loss',patience=1), ModelCheckpoint("hatespeech_sentence_embedding.model",monitor="val_loss",save_best_only=True)]
    ...
    for epoch in range(EPOCH):
        ...
        if val_loss < min:
            ...
            if epoch != 0:
                import shutil
                shutil.rmtree('{}_hate_speech_model'.format(epoch))
                # shutil.rmtree('{}_hate_speech_weights'.format(epoch))
            model.save_weights('{}_hate_speech_weights'.format(epoch+1))    
            model.save('{}_hate_speech_model'.format(epoch+1))
            val_loss = 0.0
            val_count = 0
        else:
            print(val_loss,min)
